...
@section('title', __('Partner categories'))
@section('heading', __('Partner categories'))
@section('subheading', __('Service provider categories — full administration under Partner.'))
                        ...
                        <tr class="hover:bg-cream/40">
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="flex flex-wrap justify-end gap-2">
                                    ...
                                    <a href="{{ route('admin.partner.categories.edit', $category) }}" class="rounded-lg border border-primary/25 bg-primary/5 px-2.5 py-1.5 text-xs font-semibold text-primary hover:bg-primary/10">{{ __('Edit') }}</a>
                                    <form action="{{ route('admin.partner.categories.destroy', $category) }}" method="post" class="inline" onsubmit="return confirm(@json(__('Delete this category?')));">
                                        @csrf
                                        @method('DELETE')
