...
@section('subheading', __('Product category'))
    ...
    <div class="mx-auto max-w-2xl rounded-2xl border border-gray-100 bg-white p-6 shadow-card sm:p-8">
        <div class="flex flex-col gap-6 sm:flex-row sm:items-start">
            ...
            <div class="min-w-0 flex-1 space-y-3">
                ...
                <p class="inline-flex rounded-full bg-gold/15 px-3 py-1 text-xs font-semibold text-amber-900">{{ $category->type->label() }}</p>
            </div>
        </div>
        <div class="mt-8 flex flex-wrap gap-3 border-t border-gray-100 pt-6">
            <a href="{{ route('admin.product-categories.edit', $category) }}" class="btn-leaf inline-flex items-center gap-2 bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-navy">{{ __('Edit') }}</a>
            <a href="{{ route('admin.product-categories.index') }}" class="btn-leaf inline-flex items-center gap-2 border-2 border-gray-200 px-4 py-2.5 text-sm font-semibold text-navy hover:bg-cream">{{ __('Back to list') }}</a>
        </div>
    </div>
@endsection
