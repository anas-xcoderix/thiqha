...
@section('heading', __('Add product category'))
@section('subheading', __('New records use the product type by default.'))
...
@section('content')
    <div class="mx-auto max-w-2xl rounded-2xl border border-gray-100 bg-white p-6 shadow-card sm:p-8">
        <form action="{{ route('admin.product-categories.store') }}" method="post" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @include('admin.product-categories.partials.fields', ['category' => null])
