    ...
    <section class="p-6 w-full box-border">
        <main class="relative min-h-screen w-full rounded-[40px] overflow-hidden shadow-2xl flex flex-col">
            ...
            <div class="absolute inset-0 z-0">
                <img src="{{ asset('images/blog/hero.png') }}" class="min-h-screen object-cover w-full" alt="Construction">
            </div>
    ...
    <section id="blogs" class="2xl:max-w-[90%] mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="flex items-center justify-between mb-8">
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="button" class="w-[48px] h-[48px] rounded-lg flex items-center justify-center transition text-gray-500">
        ...
        <div class="cards grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 sm:gap-8">
            @for($i = 0; $i < $posts->count(); $i++)
                ...
                <a href="{{ route('blog.show', $post['id']) }}" class="block max-w-[445px] mx-auto sm:mx-0">
                    <article class="group bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-100 max-h-[518px] h-full">
                        <div class="overflow-hidden h-[292px]">
                            <img src="{{ asset($post['card_image']) }}" alt="{{ $post['title'] }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"/>
                        </div>
                        <div class="p-5">
                            <h3 class="text-base font-bold text-gray-900 text-center mb-1">{{ $post['title'] }}</h3>
                            <p class="text-xs text-gray-400 mb-3 text-center my-4">{{ $post['date'] }} · {{ $post['read_time'] }}</p>
        ...
        <div class="flex items-center justify-end gap-2 mt-12 flex-wrap">
            <span class="flex items-center px-3 py-2 gap-3 rounded-lg w-[125px] border border-[#20395d] text-sm text-gray-600 opacity-60 cursor-not-allowed">
            ...
            <span class="size-[32px] rounded-full text-[#A7A7A7] border border-[#A7A7A7] text-sm font-semibold flex items-center justify-center">1</span>
            <span class="size-[32px] rounded-full border bg-[#20395d] border-gray-300 text-sm text-white flex items-center justify-center">2</span>
            <span class="size-[32px] rounded-full text-[#A7A7A7] border border-[#A7A7A7] text-sm flex items-center justify-center">3</span>
            <span class="text-gray-300 px-1">• • • • •</span>
            <span class="size-[32px] rounded-full text-[#A7A7A7] border border-[#A7A7A7] text-sm flex items-center justify-center">10</span>
            <span class="flex items-center justify-center gap-3 px-3 py-2 rounded-lg bg-[#20395d] text-sm transition w-[125px] opacity-60 cursor-not-allowed">
